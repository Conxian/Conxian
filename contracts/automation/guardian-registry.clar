...
(define-public (register-guardian (guardian principal) (amount uint) (cxd-token <sip-010-ft-trait>))
  (begin
...
(define-public (unbond-guardian (guardian principal) (amount uint) (cxd-token <sip-010-ft-trait>))
  (begin
    (asserts! (is-eq tx-sender guardian) ERR_UNAUTHORIZED)
    (asserts! (> amount u0) ERR_INVALID_AMOUNT)
    (let ((current-opt (map-get? guardians guardian)))
      ...
      (let
        ...
        (let ((new-bond (- current-bond amount)))
          (map-set guardians guardian { bonded-cxd: new-bond, active: (> new-bond u0), tier: (compute-tier new-bond) })
          (ok true)
        )
      )
    )
...
(define-read-only (is-guardian (who principal))
  (ok (match (map-get? guardians who) g (get active g) false)))

(define-read-only (get-guardian-tier (who principal))
  (ok (match (map-get? guardians who) g (some (get tier g)) none)))
...
(define-read-only (get-accrued-rewards (who principal))
  (ok (match (map-get? guardian-rewards who) entry (get accrued entry) u0)))
