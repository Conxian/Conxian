...
(use-trait proposal-trait 'ST3PPMPR7SAY4CAKQ4ZMYC2Q9FAVBE813YWNJ4JE6.all-traits.proposal-trait)
...
;; Public Functions
(define-public (create-budget-proposal (amount uint) (token principal))
  (begin
    (let ((budget-id (var-get next-budget-id)))
      (map-set budgets { budget-id: budget-id } { proposer: tx-sender, amount: amount, token: token, executed: false })
      (var-set next-budget-id (+ budget-id u1))
      (print { event: "budget-proposal-created", budget-id: budget-id, proposer: tx-sender, amount: amount, token: token })
      (ok budget-id)
    )
  )
)
...
(define-public (execute-budget-proposal (budget-id uint))
  (begin
    ...
    (let ((budget (map-get? budgets { budget-id: budget-id })))
      ...
      (print { event: "budget-proposal-executed", budget-id: budget-id, amount: (get amount (unwrap-panic budget)), token: (get token (unwrap-panic budget)) })

      (map-set budgets { budget-id: budget-id } (merge (unwrap-panic budget) { executed: true }))
      (ok true)
    )
  )
)
