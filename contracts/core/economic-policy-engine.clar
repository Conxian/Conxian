...
(define-constant BLOCKS_PER_DAY u17280) ;; u144 * 120 for NakamotoTA
...
(define-public (execute-policy-adjustment (revenue uint) (burn-rate uint) (utilization uint))
  (begin
    (asserts! (is-owner) ERR_UNAUTHORIZED) ;; Keepers only
    (asserts! (> (- block-height (var-get last-adjustment-block)) (var-get adjustment-cooldown)) ERR_STALE_DATA)

    ;; 1. Determine Economic Mode
    (let (
      (opex-ratio (if (> revenue u0) (/ (* burn-rate u10000) revenue) u10000))
    )
...
(define-private (activate-austerity-mode)
  (begin
    ...
    (print { event: "economic-mode-change", mode: "AUSTERITY" })
...
(define-private (activate-growth-mode)
  (begin
    ...
    (print { event: "economic-mode-change", mode: "GROWTH" })
...
(define-private (activate-normal-mode)
  (begin
    ...
    (print { event: "economic-mode-change", mode: "NORMAL" })
