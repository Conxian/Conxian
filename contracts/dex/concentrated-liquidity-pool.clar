...
(define-private (mint-position-nft
  ...
  (let ((position-id (var-get next-position-id)))
    ...
    (map-set positions { position-id: position-id } {
      ...
      shares: u0,
      liquidity: liquidity,
      fee-growth-inside-0: u0,
      fee-growth-inside-1: u0,
    })
...
(define-private (burn-position-nft
    (position-id uint)
    (owner principal)
  )
  (begin
    (let ((actual-owner (unwrap! (nft-get-owner? position-nft position-id) (err u3006))))
      (asserts! (is-eq actual-owner owner) (err u3006))
    )
    (map-delete positions { position-id: position-id })
    (try! (nft-burn? position-nft position-id owner))
    (ok true)
  )
)
...
(define-private (calculate-fee-growth-inside
    (lower-tick int)
    (upper-tick int)
    (fee-growth-global-0-param uint)
    (fee-growth-global-1-param uint)
  )
  (ok {
    fee-growth-0: u0,
    fee-growth-1: u0,
...
(define-public (initialize
    ...
    (t1 principal)
    (initial-tick int)
    (rbac-contract <rbac-trait>)
  )
  (begin
    (asserts!
      (unwrap-panic (contract-call? rbac-contract has-role "contract-owner" tx-sender))
      (err u3008)
    )
    (var-set token0 t0)
    (var-set token1 t1)
    (var-set current-tick initial-tick)
    (var-set current-sqrt-price
      (unwrap! (get-sqrt-price-from-tick initial-tick) (err u3008))
    )
    (if false
      (err u0)
      (ok true)
...
(define-public (set-fee
  ...
  (begin
    (asserts!
      (is-ok (contract-call? rbac-contract has-role "contract-owner" tx-sender))
      (err u3009)
    )
    (var-set fee new-fee)
    (ok true)
...
(define-public (set-current-tick
    (new-tick int)
    (rbac-contract <rbac-trait>)
  )
  (begin
    ...
    (var-set current-sqrt-price
      (unwrap! (get-sqrt-price-from-tick new-tick) (err u3010))
    )
    (ok true)
  )
)
...
(define-public (add-liquidity
  ...
  (begin
    ...
    (let (
        ...
        (pool-sqrt-price (var-get current-sqrt-price))
      )
      (let ((liquidity (if (<= pool-sqrt-price sqrt-price-lower)
          (unwrap!
            (get-liquidity-for-amount0 sqrt-price-lower sqrt-price-upper
              amount0-desired
            )
            ERR_OVERFLOW
          )
          (if (>= pool-sqrt-price sqrt-price-upper)
            (unwrap!
              (get-liquidity-for-amount1 sqrt-price-lower sqrt-price-upper
                amount1-desired
              )
              ERR_OVERFLOW
            )
            (min
              (unwrap!
                (get-liquidity-for-amount0 pool-sqrt-price sqrt-price-upper
                  amount0-desired
                )
                ERR_OVERFLOW
              )
              (unwrap!
                (get-liquidity-for-amount1 sqrt-price-lower pool-sqrt-price
                  amount1-desired
                )
                ERR_OVERFLOW
              ))
        ...
        (let (
            (amounts (unwrap!
              (get-amounts-for-liquidity sqrt-price-lower sqrt-price-upper
                liquidity
              )
              ERR_OVERFLOW
            ))
          ...
          (asserts!
            (and (>= amount0-desired amount0-actual) (>= amount1-desired amount1-actual))
            ERR_INSUFFICIENT_LIQUIDITY
          )

          (let ((position-id (unwrap!
              (mint-position-nft tx-sender lower-tick upper-tick liquidity)
              ERR_NFT_MINT_FAILED
            )))
...
(define-private (update-tick-liquidity
    (tick int)
    (liquidity-delta uint)
    (is-add bool)
  )
  (let ((tick-data (default-to {
      ...
      (map-get? ticks { tick-id: tick })
    )))
    (map-set ticks { tick-id: tick }
      (merge tick-data {
    ...
    (let (
      ...
      (let (
        ...
        (let (
          ...
          (try! (as-contract (contract-call? token0-trait transfer amount0-returned tx-sender owner
            none
          )))
          (try! (as-contract (contract-call? token1-trait transfer amount1-returned tx-sender owner
            none
          )))
...
(define-private (swap
  ...
  (begin
    ...
    (let (
        (swap-result (unwrap! (compute-swap amount-in sqrt-price-limit is-x-for-y)
          ERR_INSUFFICIENT_LIQUIDITY
        ))
        (amount-out (get amount-out swap-result))
        (token-in (if is-x-for-y (contract-of token0-trait) (contract-of token1-trait)))
        (token-out (if is-x-for-y (contract-of token1-trait) (contract-of token0-trait)))
      )
...
(define-private (compute-swap-step
  ...
  (let (
      (result
        (if is-x-for-y
          (let ((amount-in-max (unwrap!
              (get-amount0-delta target-sqrt-price sqrt-price-current liquidity)
              ERR_OVERFLOW
            )))
            (if (>= amount-remaining amount-in-max)
              {
                amount-in: amount-in-max,
                amount-out: (unwrap!
                  (get-amount1-delta target-sqrt-price sqrt-price-current liquidity)
                  ERR_OVERFLOW
                ),
                new-sqrt-price: target-sqrt-price
              }
              (let ((new-sqrt-price (unwrap!
                  (get-next-sqrt-price-from-input amount-remaining liquidity
                    sqrt-price-current true
                  )
                  ERR_OVERFLOW
                )))
                {
                  ...
                  amount-out: (unwrap!
                    (get-amount1-delta new-sqrt-price sqrt-price-current liquidity)
                    ERR_OVERFLOW
                  ),
                  new-sqrt-price: new-sqrt-price
                }
              )
            )
          )
          (let ((amount-in-max (unwrap!
              (get-amount1-delta sqrt-price-current target-sqrt-price liquidity)
              ERR_OVERFLOW
            )))
            (if (>= amount-remaining amount-in-max)
              {
                amount-in: amount-in-max,
                amount-out: (unwrap!
                  (get-amount0-delta sqrt-price-current target-sqrt-price liquidity)
                  ERR_OVERFLOW
                ),
                new-sqrt-price: target-sqrt-price
              }
              (let ((new-sqrt-price (unwrap!
                ...
                )))
                {
                  amount-in: amount-remaining,
                  amount-out: (unwrap!
                    (get-amount0-delta new-sqrt-price sqrt-price-current liquidity)
                    ERR_OVERFLOW
                  ),
                  new-sqrt-price: new-sqrt-price
                }
    ...
    (ok {
      amount-in-step: (get amount-in result),
      amount-out-step: (get amount-out result),
      new-sqrt-price-step: (get new-sqrt-price result)
    })
  )
)

(define-private (get-next-sqrt-price-from-input
    (amount-in uint)
    (liquidity uint)
    (sqrt-price-curr uint)
    (is-x-for-y bool)
  )
  (if is-x-for-y
    (ok (/ (* liquidity sqrt-price-curr)
      (+ (* amount-in sqrt-price-curr) liquidity)
    ))
    (ok (+ sqrt-price-curr (/ amount-in liquidity)))
  )
)
...
;; @returns (response bool uint) True if successful, or an error.
;; @error ERR_OVERFLOW if a mathematical overflow occurs during fee growth calculation.
(define-private (update-fee-growth
    (amount0 uint)
    (amount1 uint)
  )
  (begin
    (var-set fee-growth-global-0
      (+ (var-get fee-growth-global-0)
        (/ amount0 (var-get total-liquidity))
      )
    )
    (var-set fee-growth-global-1
      (+ (var-get fee-growth-global-1)
        (/ amount1 (var-get total-liquidity))
      )
    )
    (ok true)
  )
...
(define-read-only (get-position-fee-growth (position-id uint))
  (let ((position (unwrap! (map-get? positions { position-id: position-id })
      ERR_POSITION_NOT_FOUND
    )))
    (ok {
      fee-growth-0: (get fee-growth-inside-0 position),
      fee-growth-1: (get fee-growth-inside-1 position),
    })
  )
)
...
(define-read-only (get-position (position-id uint))
  (ok (unwrap! (map-get? positions { position-id: position-id }) (err u3016)))
)
...
(define-public (initialize-legacy
    (t0 principal)
    (t1 principal)
    (initial-tick int)
    (rbac-contract <rbac-trait>)
...
(define-private (get-fee-growth-outside
    (lower-tick int)
    (upper-tick int)
  )
...
(define-private (get-tick-at-price (price uint))
  (ok (- (to-int (/ price u1000000)) 887272))
)
...
(define-private (get-price-at-tick (tick int))
  (ok (* u1000000 (to-uint (+ tick 887272))))
)
...
(define-private (get-liquidity-for-amounts
    (sqrt-price-a uint)
    (sqrt-price-b uint)
    (amount0 uint)
    (amount1 uint)
  ...
  (let (
      ...
      (liquidity0 (if (is-eq sqrt-price-diff u0)
        u0
        (/ (* amount0 sqrt-price-a sqrt-price-b) sqrt-price-diff)
      ))
      (liquidity1 (if (is-eq sqrt-price-diff u0)
        u0
        (/ amount1 sqrt-price-diff)
      ))
    )
...
(define-private (get-amounts-for-liquidity
    (sqrt-price-a uint)
    (sqrt-price-b uint)
    (liquidity uint)
  )
  (let (
      (amount0 (if (>= sqrt-price-a sqrt-price-b)
        u0
        (/ (* liquidity (- sqrt-price-a sqrt-price-b))
          (* sqrt-price-a sqrt-price-b)
        )
      ))
      (amount1 (if (>= sqrt-price-a sqrt-price-b)
        (* liquidity (- sqrt-price-a sqrt-price-b))
        u0
      ))
    )
...
(define-private (get-next-initialized-tick
    (tick int)
    (lte bool)
  )
...
(define-private (get-liquidity-for-amount0
    (sqrt-price-a uint)
    (sqrt-price-b uint)
    (amount0 uint)
  )
...
(define-private (get-liquidity-for-amount1
    (sqrt-price-a uint)
    (sqrt-price-b uint)
    (amount1 uint)
  )
...
(define-private (get-amount0-delta
    (sqrt-price-a uint)
    (sqrt-price-b uint)
    (liquidity uint)
  )
...
(define-private (get-amount1-delta
    (sqrt-price-a uint)
    (sqrt-price-b uint)
    (liquidity uint)
  )
  (ok (/ (* liquidity (- sqrt-price-b sqrt-price-a))
    (if (> (* sqrt-price-a sqrt-price-b) u0)
      (* sqrt-price-a sqrt-price-b)
      u1
    )))
)
