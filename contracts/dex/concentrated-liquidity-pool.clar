...
(define-constant ERR_ZERO_AMOUNT (err u1304))
(define-constant ERR_INSUFFICIENT_LIQUIDITY (err u1310))
(define-constant ERR_POSITION_NOT_FOUND (err u1320))
(define-constant ERR_NOT_INITIALIZED (err u1401))
(define-constant ERR_ALREADY_INITIALIZED (err u1400))
(define-constant ERR_INVALID_TOKEN (err u1305))
  ...
  (let (
    ...
    (let (
        ...
        (amount-out (if is-token0
          ;; token1 out
          (unwrap!
            (contract-call? .math-lib-concentrated get-amount1-delta next-p
              current-p current-l
            )
            ERR_MATH_FAIL
          )
          ;; token0 out
          (unwrap!
            (contract-call? .math-lib-concentrated get-amount0-delta current-p
              next-p current-l
            )
            ERR_MATH_FAIL
          )
        ...
        (next-tick (unwrap!
          (contract-call? .math-lib-concentrated sqrt-price-to-tick next-p)
          ERR_MATH_FAIL
        ))
      )
      (print {
      ...
      (as-contract (try! (contract-call? token-out transfer amount-out pool-principal tx-sender none)))

      ;; 4. Fee Processing (Pool -> Protocol Fee Switch)
      ;; We send the entire collected fee to the switch for this MVP configuration
      (if (> fee-amount u0)
        (begin
          (match (as-contract (contract-call? token-in transfer fee-amount pool-principal fee-switch
            none
          ))
            res
            true
          ...
          (match (contract-call? .protocol-fee-switch route-fees token-in fee-amount
            false "DEX"
          )
            res
            true
      ...
      (if is-token0
        (begin
          ...
          (asserts! (>= (var-get reserve1) amount-out) ERR_INSUFFICIENT_LIQUIDITY)
        ...
        (begin
          ...
          (asserts! (>= (var-get reserve0) amount-out) ERR_INSUFFICIENT_LIQUIDITY)
...
(define-public (decrease-liquidity
  ...
  (let (
    ...
    (let (
        (sqrt-lower (unwrap!
          (contract-call? .math-lib-concentrated tick-to-sqrt-price tick-lower)
          ERR_MATH_FAIL
        ))
        (sqrt-upper (unwrap!
          (contract-call? .math-lib-concentrated tick-to-sqrt-price tick-upper)
          ERR_MATH_FAIL
        ))
        (amount0 (unwrap!
          (contract-call? .math-lib-concentrated get-amount0-delta current-p
        ...
        (amount1 (unwrap!
          (contract-call? .math-lib-concentrated get-amount1-delta sqrt-lower
            current-p amount
          )
          ERR_MATH_FAIL
        ))
