...
;; fee-tiers: {tier-id: uint} -> { fee-rate: uint, description: string }
(define-map fee-tiers { tier-id: uint } { fee-rate: uint, description: (string-ascii 64) })

;; pool-fee-tiers: { pool-id: principal } -> { tier-id: uint }
(define-map pool-fee-tiers { pool-id: principal } { tier-id: uint })
...
(define-public (add-fee-tier (tier-id uint) (fee-rate uint) (description (string-ascii 64)))
  (begin
    ...
    (map-set fee-tiers { tier-id: tier-id } { fee-rate: fee-rate, description: description })
    (ok true)
  ))
