...
(define-public (swap (token-in principal) (token-out principal) (amount-in uint))
  (let (
      (new-token-in (unwrap! (map-get? token-map token-in) (err ERR_TOKEN_NOT_MAPPED)))
      (new-token-out (unwrap! (map-get? token-map token-out) (err ERR_TOKEN_NOT_MAPPED)))
    )
    (print 
      { 
        message: "Legacy swap function called, forwarding to new DEX", 
        token-in: new-token-in, 
        token-out: new-token-out, 
        amount-in: amount-in 
      }
    )
    (as-contract 
      (contract-call? 
        (var-get new-dex-contract) 
        swap 
        new-token-in 
        new-token-out 
        amount-in
...
(define-public (add-liquidity 
  ...
  (let (
      (new-token-a (unwrap! (map-get? token-map token-a) (err ERR_TOKEN_NOT_MAPPED)))
      (new-token-b (unwrap! (map-get? token-map token-b) (err ERR_TOKEN_NOT_MAPPED)))
    )
    (print 
      { 
        message: "Legacy add-liquidity function called, forwarding to new DEX", 
        token-a: new-token-a, 
        token-b: new-token-b, 
        amount-a: amount-a, 
        amount-b: amount-b 
...
(define-public (remove-liquidity 
  ...
  (let (
    ...
    (print 
      { 
        message: "Legacy remove-liquidity function called, forwarding to new DEX", 
        token-a: new-token-a, 
        token-b: new-token-b, 
...
(define-public (migrate-liquidity 
  (old-pool-token principal) 
  (new-pool-token principal) 
  (amount uint)
)
  (begin
    (asserts! (is-eq tx-sender (var-get contract-owner)) ERR_UNAUTHORIZED)
    ...
    (print 
      { 
        ...
        old-pool: old-pool-token, 
        new-pool: new-pool-token, 
        amount: amount 
      }
    )
    (ok true)
