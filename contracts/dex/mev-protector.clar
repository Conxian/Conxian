...
;; --- Constants ---(define-constant ERR_UNAUTHORIZED (err u6000))
...
;; Number of blocks for reveal period(define-data-var batch-auction-period uint u5)
(define-data-var last-batch-execution uint u0)
(define-data-var circuit-breaker (optional principal) none)
(define-map commitments  { user: principal, commitment-hash: (buff 32) }  { block-height: uint, revealed: bool })
...
;; Deterministic encoding: map principals to numeric indices(define-map principal-index principal uint)

;; Helper: get index for principal (defaults to 0 if not set)
(define-private (principal->index (p principal))  (default-to u0 (map-get? principal-index p)))

;; Helper: convert path of principals into a list of indices(define-private (path->to-index-list (path (list 20 principal)))  (map (lambda (p) (principal->index p)) path))
(define-private (get-commitment-hash (path (list 20 principal)) (amount-in uint) (min-amount-out (optional uint)) (recipient principal) (salt (buff 32)))  (let (    (payload {      path: (path->to-index-list path),      amount: amount-in,      min: min-amount-out,      rcpt: (principal->index recipient),      salt: salt    })  )    (sha256 (to-consensus-buff payload))  ))
(define-private (accumulate-path (p principal) (acc (buff 800)))  

;; Simplified - in production this would properly serialize the principal  acc)
(define-private (check-circuit-breaker)  (match (var-get circuit-breaker)    (breaker (let ((is-tripped (try! (contract-call? breaker is-circuit-open))))          (if is-tripped (err ERR_CIRCUIT_OPEN) (ok true))))    (ok true)  ))

;; --- Admin Functions ---(define-public (set-reveal-period (period uint))  (begin    (asserts! (is-eq tx-sender (var-get contract-owner)) ERR_UNAUTHORIZED)    (var-set reveal-period period)    (ok true)  ))
(define-public (set-batch-auction-period (period uint))  (begin    (asserts! (is-eq tx-sender (var-get contract-owner)) ERR_UNAUTHORIZED)    (var-set batch-auction-period period)    (ok true)  ))
...
;; Owner-only: assign a numeric index to a principal(define-public (set-principal-index (p principal) (idx uint))  (begin    (asserts! (is-eq tx-sender (var-get contract-owner)) ERR_UNAUTHORIZED)    (map-set principal-index p idx)    (ok true)  ))

;; --- Commit-Reveal Functions ---(define-public (commit (commitment-hash (buff 32)))  (begin    (try! (check-circuit-breaker))    (map-set commitments       { user: tx-sender, commitment-hash: commitment-hash }      { block-height: block-height, revealed: false }    )    (ok true)  ))
(define-public (reveal (path (list 20 principal)) (amount-in uint) (min-amount-out (optional uint)) (recipient principal) (salt (buff 32)))  (let (    (calculated-hash (get-commitment-hash path amount-in min-amount-out recipient salt))    (commitment-key { user: tx-sender, commitment-hash: calculated-hash })    (commitment (map-get? commitments commitment-key))  )    (asserts! (is-some commitment) ERR_COMMITMENT_NOT_FOUND)    (asserts! (not (get revealed (unwrap-panic commitment))) ERR_ALREADY_REVEALED)    (asserts! (>= block-height (+ (get block-height (unwrap-panic commitment)) (var-get reveal-period))) ERR_REVEAL_PERIOD_NOT_EXPIRED)    

;; Mark as revealed    (map-set commitments commitment-key (merge (unwrap-panic commitment) { revealed: true }))    
