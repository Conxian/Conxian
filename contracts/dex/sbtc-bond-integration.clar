...
;; Transfer payment to bond issuer        (try! (contract-call? .sbtc-token transfer purchase-price tx-sender (get issuer bond) none))                
...
;; Verify pool manager authorization or bond issuer          (asserts! (or (is-eq tx-sender (get pool-manager pool))                        (is-eq tx-sender (get issuer bond))) ERR_NOT_AUTHORIZED)                    

;; Calculate allocation amount          (let ((allocated-amount (/ (* (get total-sbtc-deposited pool) allocation-percentage) u1000000)))                        
