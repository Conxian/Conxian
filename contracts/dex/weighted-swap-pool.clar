...
(define-data-var weight-x uint u50000000)
;; 50%
(define-data-var weight-y uint u50000000)
;; 50%

;; --- Public Functions ---(define-public (create-instance (token-a principal) (token-b principal) (params (buff 256)))  (begin    (var-set token-x-trait token-a)    (var-set token-y-trait token-b)    (let ((weights (unwrap! (get-weights-from-params params) (err ERR_INVALID_WEIGHTS))))      (var-set weight-x (get weight-x weights))      (var-set weight-y (get weight-y weights))    )    (ok (as-contract tx-sender))  ))
...
(define-private (get-amount-out (amount-in uint) (balance-in uint) (balance-out uint) (weight-in uint) (weight-out uint))
  ;; out ≈ balanceOut * (1 - (balanceIn / (balanceIn + amountIn))) in 1e8 scale
  (let ((ratio (/ (* balance-in u100000000) (+ balance-in amount-in))))
    (/ (* balance-out (- u100000000 ratio)) u100000000)
  )
)
...
(define-private (pow-approx (base uint) (exp uint))
  ;; Simplified power function using integer math (1e8 fixed scale for exp)
  (if (is-eq exp u100000000)
    base
    (if (is-eq exp u0)
      u100000000
      (/ (* base exp) u100000000)
