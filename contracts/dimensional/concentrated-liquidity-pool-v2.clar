...
(define-public (initialize (token-x principal) (token-y principal) (fee uint) (initial-sqrt-price uint))
  (begin
    ...
    (var-set current-tick (unwrap! (contract-call? .concentrated-math sqrt-price-to-tick initial-sqrt-price) (err u0)))
...
(define-public (mint-position 
  ...
  (let ((position-id (try! (contract-call? (var-get position-nft-contract) mint recipient)))
    ...
    (let ((liquidity-amount (unwrap! (contract-call? .concentrated-math
                              get-liquidity-for-amounts 
                              (var-get sqrt-price-x64) 
                              (unwrap! (contract-call? .concentrated-math tick-to-sqrt-price tick-lower) (err u0))
                              (unwrap! (contract-call? .concentrated-math tick-to-sqrt-price tick-upper) (err u0))
                              amount-x-desired
...
(define-private (execute-swap (zero-for-one bool) (amount-specified uint) (sqrt-price-limit uint))
  (let ((current-sqrt-price (var-get sqrt-price-x64))
    ...
    (let ((amount-in amount-specified)
          ...
          (new-tick (unwrap! (contract-call? .concentrated-math sqrt-price-to-tick new-sqrt-price) (err u0))))
...
(define-private (calculate-output-amount (amount-in uint) (zero-for-one bool) (sqrt-price uint) (liquidity uint))
  (if zero-for-one
    (contract-call? .concentrated-math get-amount-y amount-in sqrt-price (var-get sqrt-price-x64) liquidity)
    (contract-call? .concentrated-math get-amount-x amount-in sqrt-price (var-get sqrt-price-x64) liquidity)
  )
)

(define-private (calculate-new-sqrt-price (amount-in uint) (zero-for-one bool) (sqrt-price uint) (liquidity uint))
  (contract-call? .concentrated-math get-next-sqrt-price amount-in liquidity sqrt-price zero-for-one)
)
