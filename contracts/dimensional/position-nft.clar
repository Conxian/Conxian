...
;; Initialize the contract with the address of the pool that can mint NFTs
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (asserts! (is-some (nft-get-owner? concentrated-liquidity-positions token-id)) (err u404))
    ...
    (nft-transfer? concentrated-liquidity-positions token-id sender recipient)))
...
(define-public (mint (recipient principal))
  (begin
    ...
    (let ((token-id (+ (var-get last-token-id) u1)))
      (match (nft-mint? concentrated-liquidity-positions token-id recipient)
        success (begin
