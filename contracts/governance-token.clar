...
(define-public (delegate-voting-power
  ...
  (let (
    ...
    (map-set delegated-amounts { account: delegate } { amount: (+
      (default-to u0
        (get amount (map-get? delegated-amounts { account: delegate }))
      )
      amount
    ) }
    )
    (map-set delegated-amounts { account: tx-sender } { amount: (-
      (default-to amount
        (get amount (map-get? delegated-amounts { account: tx-sender }))
      )
      amount
    ) }
    )
