...
(define-private (snapshot-voting-power
    (user principal)
    (height uint)
  )
  (let ((power (unwrap! (contract-call? .governance-token get-voting-power-at user height)
      u0
    )))
...
(define-public (propose
  ...
  (let (
      ...
      (proposer tx-sender)
      (voting-power (unwrap!
        (contract-call? .governance-token get-voting-power-at proposer
          (- block-height u1)
        )
    ...
    (begin
      ...
      (asserts! (>= voting-power (var-get proposal-threshold))
        ERR_INSUFFICIENT_VOTING_POWER
      )
...
(define-public (propose-parameter-change
  ...
  (let ((proposal-id (try! (propose title description PROPOSAL_TYPE_PARAMETER (some target-contract)
      none (some (list new-value))
    ))))
...
(define-public (vote
  ...
  (let (
    ...
    (begin
      ...
      (asserts! (>= current-block (get start-block proposal))
        ERR_PROPOSAL_NOT_ACTIVE
      )
      (asserts! (<= current-block (get end-block proposal))
        ERR_PROPOSAL_NOT_ACTIVE
      )
      (asserts! (is-eq (get state proposal) PROPOSAL_ACTIVE)
        ERR_PROPOSAL_NOT_ACTIVE
      )

      ;; Check hasn't already voted
      (asserts!
        (is-none (map-get? vote-receipts {
          proposal-id: proposal-id,
          voter: voter,
        }))
        ERR_ALREADY_VOTED
      ...
      (let ((voting-power (unwrap!
          (contract-call? .governance-token get-voting-power-at voter
        ...
        ;; Update vote tallies
        (let ((updated-proposal (if (is-eq support u1) ;; FOR
...
(define-public (execute-proposal (proposal-id uint))
  (let ((proposal (unwrap! (map-get? proposals proposal-id) ERR_PROPOSAL_NOT_FOUND)))
    (begin
      ...
      (asserts! (is-eq (get state proposal) PROPOSAL_QUEUED)
        ERR_PROPOSAL_NOT_ACTIVE
      )
      (asserts!
        (>= block-height
          (unwrap! (get queue-block proposal) ERR_PROPOSAL_NOT_ACTIVE)
        )
        ERR_PROPOSAL_NOT_ACTIVE
      )

      ;; Execute based on proposal type
      (let ((execution-result (if (is-eq (get proposal-type proposal) PROPOSAL_TYPE_PARAMETER)
...
;; @return response bool uint A response tuple indicating success or failure.
(define-public (execute-treasury-spend
    (proposal-id uint)
    (token <token>)
  )
  (let (
      (treasury-info (unwrap! (map-get? treasury-proposals proposal-id) ERR_INVALID_PARAMETERS))
    ...
    (let ((proposal (unwrap! (map-get? proposals proposal-id) ERR_PROPOSAL_NOT_FOUND)))
      ...
      (try! (contract-call? token transfer amount (as-contract tx-sender) recipient
        none
      ))
