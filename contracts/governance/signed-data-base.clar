(use-trait signed-data-base-trait .all-traits.signed-data-base-trait)
...
(define-constant ERR_UNAUTHORIZED (err u6305))
...
(define-map used-signatures { signature: (buff 65) } { used: bool, block-height: uint })
(define-map nonce-tracker { signer: principal } { nonce: uint })
...
(define-read-only (get-nonce (signer principal))
    (ok (get nonce (default-to { nonce: u0 } (map-get? nonce-tracker { signer: signer })))))

(define-private (increment-nonce (signer principal))
    (let ((current-nonce (get nonce (default-to { nonce: u0 } (map-get? nonce-tracker { signer: signer })))))
        (map-set nonce-tracker { signer: signer } { nonce: (+ current-nonce u1) })
        (ok (+ current-nonce u1))))
...
(define-public (verify-signature (message (buff 1024)) (signature (buff 65)) (signer principal))
    (let 
        (
            (is-used (default-to { used: false, block-height: u0 } (map-get? used-signatures { signature: signature })))
        )
        (asserts! (not (get used is-used)) ERR_SIGNATURE_USED)
        
        ;; In production implementation:
        ;; 1. Verify the signature cryptographically using secp256k1-recover
        ;; 2. Recover the signer address
        ;; 3. Compare with expected signer
        
        (map-set used-signatures { signature: signature } { used: true, block-height: block-height })
        (ok true)
    ))
...
(define-public (verify-with-expiry (message (buff 1024)) (signature (buff 65)) (signer principal) (expiry uint))
    (begin
        (asserts! (<= block-height expiry) ERR_EXPIRED_SIGNATURE)
        (verify-signature message signature signer)))
...
(define-private (hash-structured-data (data (buff 1024)))
    ...
    ;; In production: Implement proper EIP-712 hashing with domain separator
    (keccak256 (concat (var-get domain-separator) data)))

;; --- Read-Only Functions ---

(define-read-only (is-signature-used (signature (buff 65)))
    (ok (get used (default-to { used: false, block-height: u0 } (map-get? used-signatures { signature: signature })))))
