...
(define-map risk-assessments
  { assessment-id: uint }
  {
    ...
    assessment-factors: (list 10 {
      factor: (string-ascii 50),
      weight: uint,
      score: uint,
    }),
    ;; Assessment factors
    coverage-recommendations: (list 5 {
      type: uint,
      max-coverage: uint,
      premium-rate: uint,
    }),
...
(define-public (create-insurance-policy-nft
  ...
  (begin
    ...
    (asserts!
      (and (>= coverage-amount MIN_COVERAGE_AMOUNT) (<= coverage-amount MAX_COVERAGE_AMOUNT))
      ERR_INVALID_POLICY
    )
    (asserts!
      (and (>= policy-duration MIN_POLICY_DURATION) (<= policy-duration MAX_POLICY_DURATION))
      ERR_INVALID_POLICY
    )
...
(define-public (file-insurance-claim
  ...
  (let ((policy (unwrap! (map-get? insurance-policies { policy-id: policy-id })
    ...
    (asserts! (is-eq (get status policy) u1) ERR_POLICY_NOT_FOUND)
    ;; Must be active
    (asserts! (< block-height (get end-block policy)) ERR_POLICY_EXPIRED)
    ;; Must not be expired
    (asserts! (get premium-paid policy) ERR_PREMIUM_NOT_PAID)
    ;; Premium must be paid
    (asserts! (<= claim-amount (get coverage-amount policy))
      ERR_INSUFFICIENT_COVERAGE
    )
    (asserts!
      (> (- block-height (get last-claim-block policy)) CLAIM_WAITING_PERIOD)
      ERR_CLAIM_NOT_ELIGIBLE
    )
    ;; Must respect waiting period
...
(define-public (approve-and-pay-claim
  ...
  (begin
    ...
    (let (
        ...
        (policy (unwrap!
          (map-get? insurance-policies { policy-id: (get policy-id claim) })
          ERR_POLICY_NOT_FOUND
        ))
      )
      ;; Update claim status
      (let (
          (deductible-applied (if (< approved-amount (get deductible policy))
            approved-amount
            (get deductible policy)
          ))
        ...
        ;; Update policy
        (map-set insurance-policies { policy-id: (get policy-id claim) }
          (merge policy { total-claims-paid: (+ (get total-claims-paid policy) payment-amount) })
        )
...
(define-public (create-industry-offering
  ...
  (begin
    ...
    (let (
        (offering-id (+ (var-get next-policy-id) u40000)) ;; Use offset to avoid conflicts
        (base-premium-rate (calculate-industry-premium-rate target-industry pricing-model))
      )
      ;; Create industry offering
      (map-set industry-offerings { offering-id: offering-id } {
...
(define-public (transfer
  ...
  (let ((nft-data (unwrap! (map-get? insurance-nft-metadata { token-id: token-id })
...
(define-private (assess-user-risk-tier
    (user principal)
    (coverage-type uint)
  )
  ;; Simplified risk assessment - would integrate with risk assessment contract
  (if (is-eq coverage-type COVERAGE_SMART_CONTRACT)
    RISK_TIER_MEDIUM
    (if (is-eq coverage-type COVERAGE_LIQUIDATION)
      RISK_TIER_HIGH
      (if (is-eq coverage-type COVERAGE_SYSTEMIC_RISK)
        RISK_TIER_EXTREME
        (if (is-eq coverage-type COVERAGE_ORACLE_FAILURE)
          RISK_TIER_MEDIUM
          (if (is-eq coverage-type COVERAGE_BRIDGE_FAILURE)
            RISK_TIER_HIGH
            RISK_TIER_LOW
          )
        )
...
(define-private (calculate-premium-rate
    (coverage-type uint)
    (risk-tier uint)
  )
  (let (
      (base-rate BASE_PREMIUM_RATE)
      (risk-multiplier (if (is-eq risk-tier RISK_TIER_LOW)
        u500 ;; 0.5x for low risk
        (if (is-eq risk-tier RISK_TIER_MEDIUM)
          u1000 ;; 1x for medium risk
          (if (is-eq risk-tier RISK_TIER_HIGH)
            u2500 ;; 2.5x for high risk
            u5000
          )
        )
      ))
      ;; 5x for extreme risk
      (coverage-multiplier (if (is-eq coverage-type COVERAGE_SMART_CONTRACT)
        u1000 ;; 1x for smart contracts
        (if (is-eq coverage-type COVERAGE_LIQUIDATION)
          u1500 ;; 1.5x for liquidation
          (if (is-eq coverage-type COVERAGE_SYSTEMIC_RISK)
            u3000 ;; 3x for systemic risk
            (if (is-eq coverage-type COVERAGE_ORACLE_FAILURE)
              u1200 ;; 1.2x for oracle failure
              (if (is-eq coverage-type COVERAGE_BRIDGE_FAILURE)
                u2000 ;; 2x for bridge failure
                u1000
              )
            )
          )
...
(define-private (get-policy-exclusions (coverage-type uint))
  (if (is-eq coverage-type COVERAGE_SMART_CONTRACT)
    (list "user-error" "known-vulnerabilities" "upgrade-failures")
    (if (is-eq coverage-type COVERAGE_LIQUIDATION)
      (list "market-volatility" "user-liquidation" "forced-liquidation")
      (if (is-eq coverage-type COVERAGE_SYSTEMIC_RISK)
        (list "market-crashes" "black-swan-events" "external-failures")
        (if (is-eq coverage-type COVERAGE_ORACLE_FAILURE)
          (list "price-manipulation" "delayed-feeds" "temporary-outages")
          (if (is-eq coverage-type COVERAGE_BRIDGE_FAILURE)
            (list "network-congestion" "user-error" "temporary-suspensions")
...
(define-private (calculate-insurance-visual-tier
    (coverage-amount uint)
    (risk-tier uint)
  )
  (if (and (>= coverage-amount u100000000) (is-eq risk-tier RISK_TIER_LOW))
    u5
    (if (and (>= coverage-amount u50000000) (<= risk-tier RISK_TIER_MEDIUM))
      u4
      (if (and (>= coverage-amount u10000000) (<= risk-tier RISK_TIER_HIGH))
        u3
        u2
      )
    )
...
(define-private (calculate-claim-visual-tier (claim-amount uint))
  (if (>= claim-amount u50000000)
    u4
    (if (>= claim-amount u10000000)
      u3
      (if (>= claim-amount u1000000)
        u2
        u1
      )
    )
...
(define-private (calculate-certificate-visual-tier (coverage-amount uint))
  (if (>= coverage-amount u100000000)
    u5
    (if (>= coverage-amount u50000000)
      u4
      (if (>= coverage-amount u10000000)
        u3
        (if (>= coverage-amount u1000000)
          u2
          u1
        )
      )
...
(define-private (calculate-discount-visual-tier (discount-percentage uint))
  (if (>= discount-percentage u3000)
    u5
    (if (>= discount-percentage u2000)
      u4
      (if (>= discount-percentage u1000)
        u3
        (if (>= discount-percentage u500)
          u2
          u1
        )
      )
...
(define-private (is-valid-assessment-factors (factors (list 10 {
  factor: (string-ascii 50),
  weight: uint,
  score: uint,
})))
...
(define-private (sum-assessment-weight
    (factor {
      factor: (string-ascii 50),
      weight: uint,
      score: uint,
...
(define-private (sum-weighted-score
    (factor {
      factor: (string-ascii 50),
      weight: uint,
      score: uint,
...
(define-private (calculate-risk-score (factors (list 10 {
  factor: (string-ascii 50),
  weight: uint,
  score: uint,
})))
...
(define-private (get-assessment-special-conditions (risk-tier uint))
  (if (is-eq risk-tier RISK_TIER_LOW)
    (list "premium-discounts" "fast-claims" "enhanced-coverage")
    (if (is-eq risk-tier RISK_TIER_MEDIUM)
      (list "standard-coverage" "normal-processing")
      (if (is-eq risk-tier RISK_TIER_HIGH)
        (list "limited-coverage" "additional-monitoring")
...
(define-private (get-applicable-coverage-for-discount (discount-type uint))
  (if (is-eq discount-type u1)
    (list COVERAGE_SMART_CONTRACT COVERAGE_ORACLE_FAILURE)
    (if (is-eq discount-type u2)
      (list COVERAGE_LIQUIDATION COVERAGE_SYSTEMIC_RISK)
      (if (is-eq discount-type u3)
        (list COVERAGE_BRIDGE_FAILURE COVERAGE_SMART_CONTRACT)
        (list
          COVERAGE_SMART_CONTRACT           COVERAGE_LIQUIDATION
          COVERAGE_SYSTEMIC_RISK
          COVERAGE_ORACLE_FAILURE           COVERAGE_BRIDGE_FAILURE
        )
      )
    )
  )
...
(define-private (calculate-industry-premium-rate
    (industry (string-ascii 50))
    (pricing-model uint)
  )
  ;; Calculate industry-specific premium rate
  (let (
      (base-rate (if (is-eq pricing-model u1)
        u300 ;; Fixed pricing - 3%
        (if (is-eq pricing-model u2)
          u400 ;; Usage-based - 4%
          u500
        )
      ))
      ...
      (industry-multiplier (let ((industry-id (string-to-int? industry)))
        (if (is-eq industry-id u1)
          u800 ;; Tech - 0.8x
          (if (is-eq industry-id u2)
            u1200 ;; Finance - 1.2x
            (if (is-eq industry-id u3)
              u1500 ;; DeFi - 1.5x
              (if (is-eq industry-id u4)
                u1000 ;; Gaming - 1x
                u1000
              )
            )
...
(define-private (get-industry-special-features (industry (string-ascii 50)))
  ...
  (let ((industry-id (string-to-int? industry)))
    (if (is-eq industry-id u1)
      (list "smart-contract-audits" "tech-support" "rapid-response") ;; Tech
      (if (is-eq industry-id u2)
        (list "compliance-support" "regulatory-guidance" "financial-advisory") ;; Finance
        (if (is-eq industry-id u3)
          (list "defi-expertise" "yield-optimization" "liquidity-protection") ;; DeFi
          (if (is-eq industry-id u4)
            (list "gaming-security" "asset-protection" "player-fund-safety") ;; Gaming
...
(define-private (get-industry-eligibility-criteria (industry (string-ascii 50)))
  ...
  (let ((industry-id (string-to-int? industry)))
    (if (is-eq industry-id u1)
      (list "audited-smart-contracts" "security-certifications" "technical-team") ;; Tech
      (if (is-eq industry-id u2)
        (list "financial-licenses" "compliance-certificates" "audited-financials") ;; Finance
        (if (is-eq industry-id u3)
          (list "defi-experience" "liquidity-proofs" "community-trust") ;; DeFi
          (if (is-eq industry-id u4)
            (list
              "gaming-licenses"               "player-protection"
              "fair-play-certifications"
            )
            ;; Gaming
            (list
              "basic-requirements"               "verification-needed"
              "standard-compliance"
            )
          )
        )
...
(define-private (update-user-insurance-profile-on_policy
    (user principal)
    (policy-id uint)
    (coverage-amount uint)
    (premium-amount uint)
...
(define-private (update-user-insurance-profile-on_claim
    (user principal)
    (claim-id uint)
    (claim-amount uint)
  )
...
(define-read-only (calculate-premium-for-policy
    (coverage-type uint)
    (coverage-amount uint)
    (risk-tier uint)
  )
...
(define-read-only (verify-proof-of-insurance
    (certificate-id uint)
