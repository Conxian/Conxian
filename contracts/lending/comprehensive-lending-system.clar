...
(use-trait interest-rate-model-trait .defi-primitives.interest-rate-model-trait)
...
(define-public (set-oracle (oracle principal))
  (begin
    (asserts!
      (is-eq (contract-call? .roles has-role "contract-owner" tx-sender)
        (ok true)
      )
      ERR_UNAUTHORIZED
    )
...
(define-public (add-supported-asset
  ...
  (begin
    ...
    (asserts!
      (and (>= liquidation-threshold u5000) (<= liquidation-threshold BPS))
      ERR_INVALID_TUPLE
    )
    ...
    (let ((current (var-get supported-asset-list)))
      ...
      (var-set supported-asset-list
        (unwrap-panic (as-max-len? (append current asset) u100))
      )
    )
    (ok true)
  )
...
(define-public (remove-supported-asset (asset principal))
  (begin
    ...
    (var-set supported-asset-list
      (filter-assets (var-get supported-asset-list) asset)
    )
    (ok true)
  )
)
...
(define-private (contains-asset
    (assets (list 100 principal))
    (target principal)
  )
  (is-some (index-of assets target))
)

(define-data-var filter-target principal tx-sender)
(define-private (is-not-target (asset principal))
  (not (is-eq asset (var-get filter-target)))
)

;; Helper: remove target asset from list
(define-private (filter-assets
    (assets (list 100 principal))
    (target principal)
  )
  (begin
    (var-set filter-target target)
    (filter is-not-target assets)
  )
)
...
(define-private (get-asset-price-safe (asset principal))
  ;; Fetch price from configured oracle; return 0 on error
...
(define-private (accrue-interest
    (model-trait <interest-rate-model-trait>)
    (asset principal)
  )
  (as-contract (contract-call? model-trait accrue-interest asset))
)
...
(define-private (fold-collateral-value
    (asset principal)
    (acc-tuple {
      val: uint,
      user: principal,
  ...
  (let (
      (acc (get val acc-tuple))
      (user (get user acc-tuple))
    )
    {
      val: (if (default-to false
          (map-get? user-collateral-assets {
            user: user,
            asset: asset,
          })
        )
...
(define-private (fold-borrow-value
    (asset principal)
    (acc-tuple {
      val: uint,
      user: principal,
...
(define-public (get-total-collateral-value-in-usd-safe (user principal))
  (let ((assets (var-get supported-asset-list)))
    (ok (get val
      (fold fold-collateral-value assets {
        val: u0,
        user: user,
      })
    ))
...
(define-private (calculate-collateral-value
    (asset-tuple {
      user: principal,
      asset: principal,
    })
    ...
    (if (default-to false
      ...
      (match (map-get? supported-assets { asset: asset })
        asset-info (let (
          ...
          (+ total-value
            (/ (* balance (get collateral-factor asset-info) price) PRECISION)
          )
        )
        total-value
      )
...
(define-private (calculate-borrow-value
    (borrow-tuple {
      user: principal,
      asset: principal,
    })
...
(define-public (get-health-factor (user principal))
  (let (
      (collateral-data (calculate-weighted-collateral user))
      (collateral-value (get total-collateral-value collateral-data))
      (weighted-threshold (get-weighted-liquidation-threshold {
        total-collateral-value: (get total-collateral-value collateral-data),
        total-threshold-value: (get total-threshold-value collateral-data)
      }))
...
(define-private (fold-health-factor
    (asset principal)
    (acc-tuple {
      total-collateral-value: uint,
      total-threshold-value: uint,
      user: principal,
    })
  )
  (let (
      (total-collateral-value (get total-collateral-value acc-tuple))
      (total-threshold-value (get total-threshold-value acc-tuple))
      (user (get user acc-tuple))
    )
    (if (default-to false
        (map-get? user-collateral-assets {
      ...
      (match (map-get? supported-assets { asset: asset })
        info (let (
          ...
          {
            total-collateral-value: (+ total-collateral-value value-usd),
            total-threshold-value: (+ total-threshold-value (* value-usd threshold)),
            user: user,
          }
        )
        acc-tuple
...
(define-private (calculate-weighted-collateral (user principal))
  (let ((assets (var-get supported-asset-list)))
    (fold fold-health-factor assets {
      total-collateral-value: u0,
        ...
        asset-info (let (
          ...
          )
          {
            ...
            total-threshold-value: (+ (get total-threshold-value accumulator)
              (/ (* collateral-value (get liquidation-threshold asset-info))
                PRECISION
              )),
          }
        )
...
(define-private (get-weighted-liquidation-threshold (collateral-data {
  total-collateral-value: uint,
  total-threshold-value: uint,
}))
  (if (> (get total-collateral-value collateral-data) u0)
    ;; Scale to PRECISION: (sum(value * threshold_bps)/sum(value)) * PRECISION / BPS
    (/ (* (get total-threshold-value collateral-data) PRECISION)
      (* (get total-collateral-value collateral-data) BPS)
    )
    u0
  )
)
...
(define-private (call-circuit-breaker-success)
  (contract-call? .circuit-breaker record-success LENDING_SERVICE)
)

(define-private (call-circuit-breaker-failure)
  (contract-call? .circuit-breaker record-failure LENDING_SERVICE)
)

(define-private (check-circuit-breaker)
  (match (contract-call? .circuit-breaker check-circuit-state LENDING_SERVICE)
    success (ok true)
    error (err ERR_CIRCUIT_BREAKER_OPEN)
  )
)
...
(define-public (supply
  ...
  (match (supply-internal asset amount model-trait)
    success (begin
...
(define-private (supply-internal
  ...
  (begin
    ...
    (let ((asset-principal (contract-of asset)))
      (asserts! (is-some (map-get? supported-assets { asset: asset-principal }))
        ERR_INVALID_ASSET
      )
      (try! (accrue-interest model-trait asset-principal))
...
(define-public (withdraw
  ...
  (match (withdraw-internal asset amount model-trait)
    success (begin
      (try! (call-circuit-breaker-success))
      (ok success)
    )
      ...
      (let ((current-balance (default-to u0
        ...
        (let ((new-balance (- current-balance amount)))
          ...
          (if (is-eq new-balance u0)
            (map-delete user-collateral-assets {
              user: tx-sender,
              asset: asset-principal,
            })
            true
          ...
          (let ((health-factor (try! (get-health-factor tx-sender))))
            (asserts! (>= health-factor (var-get min-health-factor-precision))
...
(define-public (borrow
  ...
  (match (borrow-internal asset amount model-trait)
    success (begin
      (try! (call-circuit-breaker-success))
      (ok success)
    )
  ...
  (begin
    ...
    (let ((asset-principal (contract-of asset)))
      ...
      (let ((health-factor (try! (get-health-factor tx-sender))))
        ...
        (let ((available-liquidity (unwrap! (contract-call? asset get-balance (as-contract tx-sender))
            ERR_TRANSFER_FAILED
          )))
          (asserts! (>= available-liquidity amount) ERR_INSUFFICIENT_LIQUIDITY)
          (let ((current-borrow-balance (default-to u0
              (get balance
                (map-get? user-borrow-balances {
                  user: tx-sender,
                  asset: asset-principal,
            ...
            (map-set user-borrow-balances {
              user: tx-sender,
              asset: asset-principal,
            } { balance: (+ current-borrow-balance amount) }
            )
            (try! (as-contract (contract-call? asset transfer amount tx-sender tx-sender none)))
            (ok true)
          )
...
(define-private (liquidate-internal
    ...
    (borrowed-asset <sip-010-ft-trait>)
    (amount uint)
  )
  (begin
    (try! (check-not-paused))
    (let (
        ...
        (borrowed-asset-info (unwrap! (map-get? supported-assets { asset: borrowed-asset-principal })
          ERR_INVALID_ASSET
        ))
        (collateral-asset-info (unwrap!
          (map-get? supported-assets { asset: collateral-asset-principal })
          ERR_INVALID_ASSET
        ))
        ...
        (amount-to-seize (/ (* (/ (* amount borrowed-price) PRECISION) (+ BPS liquidation-bonus))
          collateral-price
        ))
        (borrower-collateral (default-to u0
          (get balance
            (map-get? user-supply-balances {
              user: borrower,
              asset: collateral-asset-principal,
            })
          )))
        (borrower-borrow-balance (default-to u0
          (get balance
            (map-get? user-borrow-balances {
              user: borrower,
              asset: borrowed-asset-principal,
            })
          )))
      )
      (asserts! (< health-factor (var-get min-health-factor-precision))
      ...
      (asserts! (>= borrower-collateral amount-to-seize)
        ERR_INSUFFICIENT_COLLATERAL
      )
      (try! (contract-call? borrowed-asset transfer amount tx-sender
        (as-contract tx-sender) none
      ))
      (map-set user-borrow-balances {
        user: borrower,
        asset: borrowed-asset-principal,
      } { balance: (- borrower-borrow-balance amount) }
      )
      (map-set user-supply-balances {
        user: borrower,
        asset: collateral-asset-principal,
      } { balance: (- borrower-collateral amount-to-seize) }
      )
      (try! (as-contract (contract-call? collateral-asset transfer amount-to-seize tx-sender
        tx-sender none
      )))
      (ok true)
    )
...
(define-public (repay
  ...
  (match (repay-internal asset amount model-trait)
    success (begin
      (try! (call-circuit-breaker-success))
      (ok success)
    )
  ...
  (begin
    ...
    (let ((asset-principal (contract-of asset)))
      ...
      (let (
        ...
        (try! (contract-call? asset transfer repay-amount tx-sender
          (as-contract tx-sender) none
        ))
        (map-set user-borrow-balances {
          user: tx-sender,
          asset: asset-principal,
        } { balance: (- current-borrow repay-amount) }
        )
...
(define-public (liquidate
  ...
  (begin
    (asserts! (is-eq tx-sender (var-get loan-liquidation-manager-contract))
      ERR_UNAUTHORIZED
    )
    (try! (check-not-paused))
    ...
    (let ((health (try! (get-health-factor borrower))))
      ...
      (let (
          (repay-asset-principal (contract-of repay-asset))
          (collateral-asset-principal (contract-of collateral-asset))
        )
        (try! (accrue-interest model-trait repay-asset-principal))
        (try! (accrue-interest model-trait collateral-asset-principal))
        ;; Transfer repay amount from liquidator to protocol
        (try! (as-contract (contract-call? repay-asset transfer repay-amount liquidator
          (as-contract tx-sender) none
        )))

        ;; Reduce borrower borrow balance
        (let (
            (borrow-current (default-to u0
              (get balance
                (map-get? user-borrow-balances {
                  user: borrower,
                  asset: repay-asset-principal,
                })
              )))
          ...
          (map-set user-borrow-balances {
            user: borrower,
            asset: repay-asset-principal,
          } { balance: (- borrow-current repay-effective) }
          )
          ...
          (match (map-get? supported-assets { asset: collateral-asset-principal })
            info (let (
                ...
                (collateral-balance (default-to u0
                  (get balance
                    (map-get? user-supply-balances {
                      user: borrower,
                      asset: collateral-asset-principal,
              ...
              (map-set user-supply-balances {
                user: borrower,
                asset: collateral-asset-principal,
              } { balance: (- collateral-balance seize-final) }
              )
              (try! (as-contract (contract-call? collateral-asset transfer seize-final
                (as-contract tx-sender) liquidator none
              )))
              (record-user-metrics borrower)
              (ok {
                repaid: repay-effective,
                seized: seize-final,
              })
            )
