...
(define-constant BLOCKS_PER_YEAR u756864000) ;; Nakamoto-adjusted: (365 * 24 * 60 * 60) / 5
...
(define-private (check-is-lending-system)
  (begin
    (asserts!
      (is-eq tx-sender
        (unwrap! (var-get lending-system-contract) ERR_LENDING_SYSTEM_NOT_SET)
      )
      ERR_UNAUTHORIZED
    )
    (ok true)
...
(define-public (set-interest-rate-model
  ...
  (begin
    ...
    (map-set interest-rate-models { asset: asset } {
    ...
    (match (map-get? interest-rate-models { asset: asset })
      model (let (
        ...
        (if (<= utilization kink)
          ...
          (+ base-rate (/ (* kink multiplier) PRECISION)
...
(define-read-only (get-supply-rate-per-year (asset principal))
  (let (
    ...
    (/ (* borrow-rate utilization (- PRECISION RESERVE_FACTOR))
      (* PRECISION PRECISION)
    )
  )
)
    ...
    market (let ((blocks-elapsed (- block-height (get last-update-block market))))
      (if (is-eq blocks-elapsed u0)
        ...
        (let (
          ...
          (let (
              (new-borrow-index (+ (get borrow-index market)
                (/ (* (get borrow-index market) simple-interest-factor) PRECISION)
              ))
              ...
              (new-supply-index (+ (get supply-index market)
                (/ (* (get supply-index market) supply-interest-factor) PRECISION)
              ))
            ...
            (let ((updated-market (merge market {
...
(define-public (initialize-market (asset principal))
  (begin
    ...
    (asserts! (is-none (map-get? market-state { asset: asset }))
      ERR_INVALID_PARAMETER
    )
