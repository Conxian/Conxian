...
(define-private (sqrt-iter-internal
    (x uint)
    (n uint)
  )
  (ok x)
...
(define-private (ln-taylor-internal
    (x uint)
    (result uint)
    (term uint)
    (n uint)
...
(define-private (exp-taylor-internal
    (x uint)
    (result uint)
    (term uint)
    (n uint)
...
(define-read-only (calculate-swap-amount-out-x-to-y
...
(define-read-only (calculate-new-sqrt-price-x-to-y
    (sqrt-price-current uint)
    (liquidity uint)
    (amount-in uint)
  )
...
(define-read-only (calculate-swap-amount-out-y-to-x
    (sqrt-price-current uint)
    (liquidity uint)
    (amount-in uint)
  )
...
(define-read-only (calculate-new-sqrt-price-y-to-x
    (sqrt-price-current uint)
    (liquidity uint)
    (amount-in uint)
  )
...
(define-read-only (is-safe-uint (n uint))
  (< n u340282366920938463463374607431768211455)
)
;; Max safe uint
