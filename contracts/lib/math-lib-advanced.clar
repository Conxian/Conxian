...
(define-private (ln2-approx (t uint) (t-sq uint) (n uint) (p uint) (i uint))
  (if (>= i u10)  ;; 10 iterations for good precision
    (ok p)
    (let* (
      (term (unwrap! (div-down (unwrap! (pow-fixed t-sq n) (err ERR_OVERFLOW)) (unwrap! (safe-mul u2 (unwrap! (safe-add i u1) (err ERR_OVERFLOW))) (err ERR_OVERFLOW))) (err ERR_UNDERFLOW)))
...
(define-read-only (ln2-fixed)
  (ok 6931471805599453094)  ;; ln(2) * 1e18
)
...
(define-read-only (safe-mul (a uint) (b uint))
  (let ((product (* a b)))
    (if (or (is-eq a u0) (is-eq b u0) (<= product a) (<= product b))
      (ok product)
      (err ERR_OVERFLOW)
    )
  )
