...
(define-public (detect-precision-loss (operation (string-ascii 32)) (input-a uint) (input-b uint) (expected uint) (actual uint))
  (let ((precision-loss (abs-diff expected actual))
        (precision-loss-bps (if (is-eq expected u0) u0 (/ (* (abs-diff expected actual) u10000) expected))))
    (begin
      ...
      (if (<= precision-loss-bps MAX_PRECISION_LOSS_BPS)
        (begin
          ...
          (ok (tuple (operation operation) (precision-loss precision-loss) (precision-loss-bps precision-loss-bps) (within-threshold true))))
...
(define-read-only (get-precision-stats)
  (let ((total-ops (var-get total-operations))
        ...
        (success-rate (if (is-eq total-ops u0) u0 (/ (* ops-within u10000) total-ops))))
    (tuple
