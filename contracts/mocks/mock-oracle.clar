...
(define-read-only (get-price (token principal))
  (match (map-get? mock-prices {token: token})
    price-data => 
      (ok (get price (unwrap-panic price-data)))
    _ => (err ERR_TOKEN_NOT_FOUND)
  )
)
...
(define-public (update-price (token principal) (price uint))
  (begin
    ...
    (match (map-get? mock-prices {token: token})
      price-data =>
        (let ((current-data (unwrap-panic price-data)))
          (map-set mock-prices 
            {token: token}
            (merge current-data {
              price: price,
...
(define-read-only (get-last-updated (token principal))
  (match (map-get? mock-prices {token: token})
    price-data => 
      (ok (get last-updated (unwrap-panic price-data)))
    _ => (err ERR_TOKEN_NOT_FOUND)
  )
)
...
(define-read-only (get-deviation-threshold (token principal))
  (match (map-get? mock-prices {token: token})
    price-data => 
      (ok (get deviation (unwrap-panic price-data)))
    _ => (err ERR_TOKEN_NOT_FOUND)
  )
)
...
(define-read-only (is-price-stale (token principal))
  (match (map-get? mock-prices {token: token})
    price-data => 
      (let ((last-updated (get last-updated (unwrap-panic price-data))))
        (ok (> (- block-height last-updated) u1440))  ;; Stale if >1 day old
      )
    _ => (ok true)  ;; Consider non-existent price as stale
  )
)

(define-read-only (get-feed-count (token principal))
  (match (map-get? mock-prices {token: token})
    _ => (ok u1)  ;; Mock always has 1 feed
    _ => (ok u0)
  )
)
