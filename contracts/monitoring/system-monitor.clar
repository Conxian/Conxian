...
(define-public (log-event
  ...
  (let (
    ...
    (match (map-get? component-health { component: component })
        health-data (map-set component-health { component: component }
            (merge health-data {
        ...
        (map-set component-health { component: component } {
            status: (if (>= severity SEVERITY_ERROR)
                STATUS_ISSUE
                STATUS_HEALTHY
            ),
            last-updated: current-block,
    ...
    (map-set component-events { component: component, index: event-id } event-id)
...
(define-read-only (get-events
    ...
    (begin
        (asserts! (<= limit u100) ERR_INVALID_LIMIT)
        (ok (unwrap-panic (as-max-len? (list) u100)))
    )
)
