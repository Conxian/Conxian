...
(define-map feed-count
  { token: principal }
  uint
)
(define-map feed-index
  {
    token: principal,
    idx: uint,
  }
...
(define-public (emergency-price-override
    (token principal)
    (price uint)
  )
  (begin
...
(define-public (remove-price-feed
    (asset principal)
    (feed principal)
  )
  (begin
    (try! (only-admin))
    (match (map-get? feed-rev-index {
      token: asset,
      feed: feed,
    })
      idx (let ((count (default-to u0 (map-get? feed-count { token: asset }))))
        (asserts! (> count u0) (err ERR_FEED_NOT_FOUND))
        (let ((last-idx (- count u1)))
          (if (is-eq idx last-idx)
            (begin
              (map-delete feed-index {
                token: asset,
                idx: last-idx,
              })
              (map-delete feed-rev-index {
                token: asset,
                feed: feed,
              })
              (map-delete feed-set {
  ...
  (begin
    ...
    (let ((current-block u0))
      (match (map-get? price-data { token: asset })
        existing
        (begin
          (asserts! (is-price-deviation-valid? (get price existing) price)
...
(define-private (is-price-deviation-valid?
    (old-price uint)
    (new-price uint)
  )
  (let (
...
(define-private (sum-feed-prices
    (asset principal)
    (i uint)
    (n uint)
    (sum uint)
