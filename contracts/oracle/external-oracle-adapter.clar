...
(define-public (initialize
  ...
  (begin
    ...
    (asserts! (and (>= manipulation-thresh u0) (<= manipulation-thresh u10000))
...
(define-public (add-oracle-source
  ...
  (begin
    ...
    (asserts! (not (is-some (map-get? oracle-sources { source-id: source-id })))
  ...
  (begin
    ...
    (let (
      ...
      (let (
        ...
        (asserts! (not (check-price-manipulation-risk valid-prices median-price))
        ...
        (map-set aggregated-prices { asset-pair: asset-pair } {
          ...
          timestamp: (get timestamp
            (unwrap! (element-at sorted-prices (/ (len sorted-prices) u2))
              (err u0)
            )),
