...
(define-public (stake (amount uint) (token <sip-010-ft-trait>))
    (let (
        (sender tx-sender)
        ...
        ;; Update State
        (let ((current-stake (default-to { amount: u0, cooldown-start: u0 } (map-get? user-stakes { user: sender }))))
            (map-set user-stakes 
                { user: sender }
                { 
...
(define-public (unstake (amount uint) (token <sip-010-ft-trait>))
    (let (
        (sender tx-sender)
        (token-principal (contract-of token))
        (current-stake (unwrap! (map-get? user-stakes { user: sender }) ERR_INSUFFICIENT_STAKE))
        ...
        (asserts! (>= block-height (+ (get cooldown-start current-stake) COOLDOWN_BLOCKS)) ERR_COOLDOWN_ACTIVE)
...
(define-public (slash-funds (amount uint) (token <sip-010-ft-trait>) (recipient principal))
    (begin
        (asserts! (is-eq tx-sender (var-get governance-contract)) ERR_NOT_GOVERNANCE)
        (asserts! (is-eq (contract-of token) (var-get staking-token)) ERR_UNAUTHORIZED)
...
(define-public (governance-withdraw (amount uint) (token <sip-010-ft-trait>) (recipient principal))
    (begin
        (asserts! (is-eq tx-sender (var-get governance-contract)) ERR_NOT_GOVERNANCE)
        ;; Prevent withdrawing the Staked Token unless it's a slash (use slash-funds for that)
        (asserts! (not (is-eq (contract-of token) (var-get staking-token))) ERR_UNAUTHORIZED)
