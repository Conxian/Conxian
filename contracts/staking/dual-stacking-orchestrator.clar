...
(impl-trait dual-stacking-trait)
...
(define-public (initialize (sbtc <sip-010-ft-trait>) (operator principal) (fee-bps uint))
  (begin
    (asserts! (is-eq tx-sender (var-get owner)) ERR_UNAUTHORIZED)
    (var-set sbtc sbtc)
    (var-set operator operator)
    (var-set fee-bps fee-bps)
    (var-set fee-recipient operator)
    (ok true)
  )
)
...
(define-public (record-delegations (cycle-id uint) (entries (list 200 (tuple (user principal) (amount uint)))))
  (begin
    ...
    (let (
      (added-total (fold entries u0 (lambda (e acc)
        (let ((user (get user e)) (amt (get amount e))
              (prev (default-to u0 (map-get? user-delegations {cycle-id: cycle-id, user: user}))))
          (map-set user-delegations {cycle-id: cycle-id, user: user} (+ prev amt))
          (+ acc amt)
        )
      )))
      (totals (default-to { total-delegated: u0, reward: u0 } (map-get? cycle-totals {cycle-id: cycle-id})))
    )
      (map-set cycle-totals {cycle-id: cycle-id} { total-delegated: (+ (get total-delegated totals) added-total), reward: (get reward totals) })
      (ok true)
    )
  )
)
  ...
  (begin
    ...
    (let (
      ...
      (let (
        ...
        (net (- amount fee))
        (keys (map-get-keys user-delegations {cycle-id: cycle-id}))
        (allocated (fold keys u0 (lambda (k acc)
          (let ((u (get user k))
                (d (unwrap-panic (map-get? user-delegations k)))
                (share (/ (* net d) total))
                (prev (default-to u0 (map-get? claimable {user: u}))))
            (map-set claimable {user: u} (+ prev share))
            (+ acc share)
          )
        )))
        (fr (var-get fee-recipient))
        (prev-fee (default-to u0 (map-get? claimable {user: fr})))
      )
        (map-set claimable {user: fr} (+ prev-fee fee))
        (map-set cycle-totals {cycle-id: cycle-id} { total-delegated: total, reward: (+ (get reward totals) net) })
        (ok true)
      )
    )
  )
