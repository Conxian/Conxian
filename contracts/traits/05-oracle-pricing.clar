...
(define-trait oracle-aggregator-v2-trait
  (
    (get-real-time-price (principal) (response uint uint))
    (get-twap (principal uint) (response uint uint))
    (add-oracle-source (principal principal) (response bool uint))
    (remove-oracle-source (principal principal) (response bool uint))
  )
)
...
(define-trait price-initializer-trait
  (
    (initialize-price (principal uint) (response bool uint))
    (get-initial-price (principal) (response uint uint))
  )
)
...
(define-trait dimensional-oracle-trait
  (
    (get-dimensional-price (principal (string-ascii 32)) (response uint uint))
    (update-dimensional-metrics (principal (string-ascii 32) uint) (response bool uint))
  )
)
