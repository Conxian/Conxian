...
(define-trait oracle-aggregator-trait
  (
    (register-oracle (oracle-id principal) (weight uint) (response bool (err uint)))
    (submit-price (asset principal) (price uint) (response bool (err uint)))
    (get-aggregated-price (asset principal) (response uint (err uint)))
    (get-price-with-confidence (asset principal) (response (tuple (price uint) (confidence uint) (sources uint)) (err uint)))
    (get-twap-with-period (asset principal) (period uint) (response uint (err uint)))
    (detect-price-manipulation (asset principal) (current-price uint) (response bool (err uint)))
    (detect-statistical-outlier (asset principal) (price uint) (response bool (err uint)))
    (set-circuit-breaker-contract (principal) (response bool (err uint)))
  )
