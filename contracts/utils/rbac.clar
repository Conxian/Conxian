...
(define-public (assign-role
  ...
  (begin
    ...
    (asserts! (is-none (map-get? roles { role-name: role-name }))
      ERR_ROLE_ALREADY_ASSIGNED
    )
...
(define-public (revoke-role (role-name (string-ascii 32)))
  (begin
    (asserts! (is-contract-owner-p) ERR_UNAUTHORIZED)
    (asserts! (is-some (map-get? roles { role-name: role-name }))
      ERR_ROLE_NOT_ASSIGNED
    )
...
(define-read-only (has-role (role-name (string-ascii 32)))
  (ok (is-eq tx-sender
    (get authorized-principal
      (unwrap! (map-get? roles { role-name: role-name }) ERR_ROLE_NOT_ASSIGNED)
    )))
)
