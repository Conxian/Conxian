...
(define-trait yield-aggregator-trait
  (
    ;; @desc Allocates a specified amount of sBTC to a yield-generating strategy.
    ...
    ;; @param strategy principal The principal of the strategy contract.
    ;; @returns (response uint uint) A response containing the net yield harvested or an error.
  ...
  (begin
    ...
    (match (map-get? strategy-allocations strategy)
      ...
      (let ((yield-earned (- (get current-value allocation) (get allocated-amount allocation))))
        (var-set total-yield-generated (+ (var-get total-yield-generated) yield-earned))
