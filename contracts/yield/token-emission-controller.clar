...
(define-constant EMISSION_PERIOD u6307200) ;; 1 year in Nakamoto blocks (5s blocks)
...
(define-constant TIMELOCK_BLOCKS u241920) ;; ~2 weeks in Nakamoto blocks (120,960 * 2)
(define-constant EMERGENCY_TIMELOCK u17280) ;; ~1 day in Nakamoto blocks (17,280 blocks)
  ...
  (begin
    ...
    (match (var-get cxd-contract)
      cxd-addr (map-set token-emission-limits cxd-addr {
        max-annual-bps: CXD_MAX_ANNUAL_INFLATION,
    ...
    (match (var-get cxvg-contract)
      cxvg-addr (map-set token-emission-limits cxvg-addr {
        max-annual-bps: CXVG_MAX_ANNUAL_INFLATION,
        max-single-mint-bps: MAX_SINGLE_MINT_BPS,
      })
      true
    )
    ...
    (match (var-get cxlp-contract)
      cxlp-addr (map-set token-emission-limits cxlp-addr {
        max-annual-bps: CXLP_MAX_ANNUAL_INFLATION,
        max-single-mint-bps: MAX_SINGLE_MINT_BPS,
      })
      true
    )
    ...
    (match (var-get cxtr-contract)
      cxtr-addr (map-set token-emission-limits cxtr-addr {
        max-annual-bps: CXTR_MAX_ANNUAL_INFLATION,
        max-single-mint-bps: MAX_SINGLE_MINT_BPS,
      })
      true
    )
...
(define-public (controlled-mint
  ...
  (let (
    ...
    (begin
      ...
      (let ((limits (unwrap! (map-get? token-emission-limits token-principal)
          (err ERR_INVALID_TOKEN)
        )))
        (maybe-advance-epoch)
        (let (
            (current-supply (get-token-supply-from-trait token-sip010))
            (epoch-data (default-to {
              minted: u0,
              supply-at-start: current-supply,
            }
              (map-get? epoch-emissions {
                token: token-principal,
                epoch: current-epoch-num,
              })
            ))
            (annual-cap (/ (* (get supply-at-start epoch-data) (get max-annual-bps limits))
              u10000
            ))
          ...
          (asserts! (<= (+ (get minted epoch-data) amount) annual-cap)
            (err ERR_EMISSION_CAP_EXCEEDED)
          )
          (try! (as-contract (contract-call? token-mintable mint amount recipient)))
          (map-set epoch-emissions {
            token: token-principal,
            epoch: current-epoch-num,
          } {
...
(define-public (propose-limit-change
  ...
  (let (
      (governance (unwrap! (var-get governance-contract) (err ERR_UNAUTHORIZED)))
      (timelock (if is-emergency
        EMERGENCY_TIMELOCK
        TIMELOCK_BLOCKS
      ))
    )
    (begin
      ...
      (asserts! (and (<= new-annual-bps u20000) (<= new-single-mint-bps u1000))
...
(define-public (vote-on-limit-change
    (token-contract principal)
    (support bool)
    (voting-power uint)
  )
...
(define-public (execute-limit-change (token-contract principal))
  (match (map-get? pending-limit-changes token-contract)
    change-info (begin
      ...
      (asserts!
        (>= block-height
          (+ (get proposed-at change-info) (get timelock-blocks change-info))
        )
        (err ERR_TIMELOCK_NOT_ELAPSED)
      )

      ;; Check supermajority requirement
      (let ((support-ratio (if (> (get total-voting-power change-info) u0)
          (/ (* (get votes-for change-info) u10000)
            (get total-voting-power change-info)
          )
          u0
        )))
        (asserts! (>= support-ratio SUPERMAJORITY_THRESHOLD)
          (err ERR_GOVERNANCE_THRESHOLD_NOT_MET)
        )
...
(define-public (emergency-reduce-limits
    (token-contract principal)
    (new-annual-bps uint)
    (new-single-mint-bps uint)
  )
  (let ((current-limits (unwrap! (map-get? token-emission-limits token-contract)
      (err ERR_INVALID_TOKEN)
    )))
    (begin
      (asserts! (is-eq tx-sender (var-get contract-owner)) (err ERR_UNAUTHORIZED))
      ...
      (asserts!
        (and
          (<= new-annual-bps (get max-annual-bps current-limits))
