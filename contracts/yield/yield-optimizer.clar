...
(define-public (update-metrics
    ...
    (let ((current (unwrap! (map-get? strategies { strategy: strategy })
...
(define-public (refresh-strategy (strategy <strategy-trait>))
    (let (
            ...
            (risk (unwrap! (contract-call? strategy get-risk-score)
                ERR_INVALID_STRATEGY
            ))
        )
...
(define-public (optimize-allocation (vault <vault-trait>) (amount uint) (cb <circuit-breaker-trait>))
    (let (
