...
* **Alignment:**
  * **Strong:** The inclusion of `enterprise-facade` and `compliance-hooks` directly addresses institutional needs (KYC/AML, tiered limits).
...
1. **Verify Enterprise Facade:** Ensure `compliance-hooks` are actually called in the `swap` functions of the pools. (Currently, they might be disconnected).

## 7. Unified Context Diagram (Conceptual)

```
[Institutional Client] --> [Enterprise Facade (KYC/Limits)]
                                  |
                                  v
