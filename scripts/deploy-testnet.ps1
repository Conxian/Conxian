...
(Get-Content "$PSScriptRoot/../Clarinet.toml") -replace 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM', $deployerAddress | Set-Content "$PSScriptRoot/../Clarinet.toml"
...
Get-ChildItem -Path "$PSScriptRoot/../contracts" -Filter "*.clar" -Recurse | ForEach-Object {
    (Get-Content $_.FullName) -replace 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM', $deployerAddress | Set-Content $_.FullName
}
