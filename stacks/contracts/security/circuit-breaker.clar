...
(define-private (should-circuit-open (stats {
  success-count: uint,
  failure-count: uint,
  last-updated: uint,
  is-open: bool,
...
(define-private (should-circuit-close (stats {
  success-count: uint,
  failure-count: uint,
  last-updated: uint,
  is-open: bool,
...
(define-private (update-rate-limit (stats {
  success-count: uint,
  failure-count: uint,
  last-updated: uint,
  is-open: bool,
...
(define-private (update-circuit-state (stats {
  success-count: uint,
  failure-count: uint,
  last-updated: uint,
  is-open: bool,
...
(define-read-only (get-circuit-state (operation (string-ascii 64)))
  (let ((stats (default-to
                  (get-default-stats block-height)
                  (map-get? operation-stats {operation: operation})
                ))
        (failure-rate (calculate-failure-rate
                        (get success-count stats)
                        (get failure-count stats)
                      )))
