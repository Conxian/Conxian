...
describe('Cross Protocol Integrator Tests', () => {
  ...
  beforeEach(() => {
    ...
    simnet.deployContract('mock-token-a', `(define-fungible-token mock-token-a) (define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buffer 32)))) (begin (ft-transfer? mock-token-a amount sender recipient) (ok true)))`, deployer);
    simnet.deployContract('mock-yield-optimizer', `(define-public (find-best-strategy (a principal) (b principal)) (ok (some tx-sender)))`, deployer);
    simnet.deployContract('mock-circuit-breaker', `(define-data-var circuit-open bool false) (define-public (set-circuit-open (status bool)) (ok (var-set circuit-open status))) (define-read-only (is-circuit-open) (ok (var-get circuit-open)))`, deployer);
    simnet.deployContract('mock-rbac', `(define-data-var owner principal tx-sender) (define-read-only (has-role (role (string-ascii 32))) (ok (is-eq role "contract-owner")))`, deployer);
