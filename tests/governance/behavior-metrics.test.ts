...
describe('Behavior Metrics & Reputation System', () => {
  ...
  describe('Behavior Metrics Read Functions', () => {
    ...
    it('returns default governance behavior for new users', () => {
      ...
      expect((behavior.result as any)).toHaveClarityType(ClarityType.ResponseOk);
      const data = (behavior.result as any).value.data;
  ...
  describe('Lending Behavior Recording', () => {
    it('records healthy lending behavior', () => {
      ...
      const data = (behavior.result as any).value.data;
      expect(data['average-health-factor']).toStrictEqual(Cl.uint(1500)); // (0*9 + 15000) / 10
  ...
  describe('MEV Protection Behavior Recording', () => {
    ...
    it('increases MEV awareness score with usage', () => {
      ...
      expect(data['mev-awareness-score']).toStrictEqual(Cl.uint(500)); // 100 per use
    });
  });
  ...
  describe('Comprehensive Behavior Dashboard', () => {
    it('returns complete behavior dashboard for user', () => {
      ...
      simnet.callPublicFn(
        'conxian-operations-engine',
        'record-lending-action',
        [Cl.standardPrincipal(wallet1), Cl.uint(15000), Cl.bool(false), Cl.bool(true)],
        deployer,
      );
      ...
      expect((dashboard.result as any)).toHaveClarityType(ClarityType.ResponseOk);
      const data = (dashboard.result as any).value.data;
      
      // Should have user principal
      expect(data['user']).toStrictEqual(Cl.standardPrincipal(wallet1));
      
      // Should have calculated score
      expect(data['overall-score']).toBeDefined();
      
      // Should have tier and multiplier
      expect(data['behavior-tier']).toBeDefined();
      expect(data['incentive-multiplier']).toBeDefined();
      
      // Should include all sub-dashboards
      expect(data['metrics']).toBeDefined();
      expect(data['governance']).toBeDefined();
      expect(data['lending']).toBeDefined();
      expect(data['mev']).toBeDefined();
      expect(data['insurance']).toBeDefined();
  ...
  describe('Behavior Tier Progression', () => {
    it('progresses from bronze to silver with good behavior', () => {
      ...
      const data = (metrics.result as any).value.data;
      const tier = data['behavior-tier'];
      const multiplier = data['incentive-multiplier'];

      // Should be at least silver tier
      expect(Number(tier.value)).toBeGreaterThanOrEqual(2);
      expect(Number(multiplier.value)).toBeGreaterThanOrEqual(125);
    });
  });
});
