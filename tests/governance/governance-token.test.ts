...
describe('Governance Token (CXVG)', () => {
  ...
  it('has correct initial metadata and zero supply', () => {
    ...
    const uri = simnet.callReadOnlyFn('governance-token', 'get-token-uri', [], deployer);
  ...
  it('allows owner to mint and updates balances and voting power', () => {
    ...
    const mint = simnet.callPublicFn('governance-token', 'mint', [
      amount,
      Cl.standardPrincipal(wallet1),
    ], deployer);
    expect(mint.result).toBeOk(Cl.bool(true));

    const balance = simnet.callReadOnlyFn('governance-token', 'get-balance', [
      Cl.standardPrincipal(wallet1),
    ], deployer);
    ...
    const power = simnet.callReadOnlyFn('governance-token', 'get-voting-power', [
      Cl.standardPrincipal(wallet1),
    ], deployer);
  ...
  it('rejects minting from non-owner', () => {
    const res = simnet.callPublicFn('governance-token', 'mint', [
      Cl.uint(1_000),
      Cl.standardPrincipal(wallet1),
    ], wallet1);
  ...
  it('supports transfers and updates voting power of sender and recipient', () => {
    ...
    const transfer = simnet.callPublicFn('governance-token', 'transfer', [
      Cl.uint(200_000),
      Cl.standardPrincipal(wallet1),
    ...
    const power1 = simnet.callReadOnlyFn('governance-token', 'get-voting-power', [
      Cl.standardPrincipal(wallet1),
    ], deployer);
    const power2 = simnet.callReadOnlyFn('governance-token', 'get-voting-power', [
      Cl.standardPrincipal(wallet2),
    ], deployer);
  ...
  it('supports delegation and undelegation of voting power', () => {
    ...
    simnet.callPublicFn('governance-token', 'mint', [
      Cl.uint(1_000),
      Cl.standardPrincipal(wallet1),
    ], deployer);
    simnet.callPublicFn('governance-token', 'mint', [
    ...
    const del = simnet.callPublicFn('governance-token', 'delegate-voting-power', [
      Cl.standardPrincipal(wallet2),
      Cl.uint(400),
    ], wallet1);
    expect(del.result).toBeOk(Cl.bool(true));

    const pow1AfterDel = simnet.callReadOnlyFn('governance-token', 'get-voting-power', [
      Cl.standardPrincipal(wallet1),
    ], deployer);
    const pow2AfterDel = simnet.callReadOnlyFn('governance-token', 'get-voting-power', [
      Cl.standardPrincipal(wallet2),
    ], deployer);
    ...
    const undel = simnet.callPublicFn('governance-token', 'undelegate-voting-power', [
      Cl.standardPrincipal(wallet2),
      Cl.uint(200),
    ], wallet1);
    expect(undel.result).toBeOk(Cl.bool(true));

    const pow1Final = simnet.callReadOnlyFn('governance-token', 'get-voting-power', [
      Cl.standardPrincipal(wallet1),
    ], deployer);
    const pow2Final = simnet.callReadOnlyFn('governance-token', 'get-voting-power', [
      Cl.standardPrincipal(wallet2),
    ], deployer);
  ...
  it('reports whether an account has voting power', () => {
    const before = simnet.callPublicFn('governance-token', 'has-voting-power', [
      Cl.standardPrincipal(wallet1),
    ], wallet1);
    ...
    const after = simnet.callPublicFn('governance-token', 'has-voting-power', [
      Cl.standardPrincipal(wallet1),
    ], wallet1);
