...
// Use the same deployer principal for these contracts as Clarinet simnet does,
// so balances and tx-sender line up with the executing contract.
const insuranceFund = () => Cl.contractPrincipal(deployer, 'conxian-insurance-fund');
const feeSwitch = () => Cl.contractPrincipal(deployer, 'protocol-fee-switch');
...
describe('Full System - Fees Routed to Insurance', () => {
  ...
  beforeEach(async () => {
    ...
    console.log("DEBUG: Test Deployer:", deployer);
    console.log("DEBUG: Hardcoded FeeSwitch:", feeSwitch());
  });
  ...
  it('routes CXD fees via protocol-fee-switch into insurance fund', () => {
    ...
    const setSplits = simnet.callPublicFn(
      "protocol-fee-switch",
      "set-fee-splits",
      [
    ...
    const setModuleFee = simnet.callPublicFn(
      "protocol-fee-switch",
      "set-module-fee",
      [
        Cl.stringAscii("DEX"),
    ...
    // Debug: Check balance of fee switch contract before routing
    const debugBal = simnet.callReadOnlyFn(
      "cxd-token",
      "get-balance",
      [feeSwitch()],
      deployer
    );
    console.log("DEBUG: Fee Switch Balance:", debugBal.result);

    // Route an already-collected fee amount held by the fee switch contract.
    const route = simnet.callPublicFn(
      "protocol-fee-switch",
      "route-fees",
      [cxdToken(), Cl.uint(feeAmount), Cl.bool(false), Cl.stringAscii("DEX")],
      deployer
    );
    ...
    const stakingBal = simnet.callReadOnlyFn(
      "cxd-token",
      "get-balance",
      [Cl.standardPrincipal(wallet2)],
      deployer
    ...
    const insuranceBal = simnet.callReadOnlyFn(
      "cxd-token",
      "get-balance",
      [insuranceFund()],
      deployer
