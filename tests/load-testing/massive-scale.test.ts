...
describe('System Performance Benchmarks', () => {
  ...
  it('Benchmarks Swap Throughput and Cost', () => {
    ...
    // 2. Fund Pool using add-liquidity to ensure reserves are updated
    simnet.callPublicFn(
      tokenCollateral,
      "mint",
    ...
    // Use add-liquidity instead of mint to update reserves
    simnet.callPublicFn(
      "concentrated-liquidity-pool",
      "add-liquidity",
      [
    ...
    simnet.callPublicFn(
      tokenCollateral,
      "mint",
      [Cl.uint(100000000000), Cl.standardPrincipal(wallet1)],
      deployer
    );
    ...
    console.log(
      `Average time per swap: ${(duration / iterations).toFixed(2)}ms`
    );
