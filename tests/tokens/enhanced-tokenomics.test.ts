...
describe("Enhanced Tokenomics System", () => {
  ...
  beforeEach(async () => {
    ...
    wallet2 = (accounts.get("wallet_2") ??
      "ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG") as string;
  });
  ...
  describe("CXD Token (Revenue Token)", () => {
    ...
    it("should allow owner or minter to mint tokens", () => {
      ...
      const receipt = simnet.callPublicFn(
        "cxd-token",
        "mint",
        [Cl.principal(wallet2), Cl.uint(mintAmount)],
        wallet1
      );
      expect(receipt.result).toBeOk(Cl.bool(true));
    ...
    it("should support token transfers and update balances", () => {
      ...
      expect(wallet1Balance.result).toBeOk(
        Cl.uint(mintAmount - transferAmount)
      );
  ...
  describe("CXTR Token (Treasury / Creator Token)", () => {
    it("should have correct initial metadata", () => {
      ...
      expect(nameResult.result).toBeOk(
        Cl.stringAscii("Conxian Treasury Token")
      );
    ...
    it("should support contributor rewards", () => {
      const receipt = simnet.callPublicFn(
        "cxtr-token",
        "mint-merit-reward",
        [Cl.principal(wallet1), Cl.uint(500_000_000), Cl.uint(1_000)],
        deployer
      );
