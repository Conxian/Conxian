...
describe('Yield Optimizer', () => {
  ...
  it('allows owner to adjust max risk', () => {
    const res = simnet.callPublicFn('yield-optimizer', 'set-max-risk', [
      Cl.uint(80),
    ], deployer);
  ...
  it('tracks a newly added strategy and updates best-strategy', () => {
    ...
    const add = simnet.callPublicFn('yield-optimizer', 'add-strategy', [
      strategy,
    ...
    const info = simnet.callReadOnlyFn('yield-optimizer', 'get-strategy', [
      strategy,
    ], deployer);
    expect(info.result).toBeSome();

    const best = simnet.callReadOnlyFn('yield-optimizer', 'get-best-strategy', [], deployer);
